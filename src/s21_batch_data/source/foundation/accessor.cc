...
std::size_t BatchData::GetClassesNumber() const {
  return classes_number_;
}

std::size_t BatchData::GetInputLength() const {
  return input_length_;
}

DataSample BatchData::GetRandomSample() const {
  std::random_device r;
...
std::size_t BatchData::GetDataSize() const {
  return classes_.size();
}

std::size_t s21::BatchData::GetMaxSteps() const {
  return max_steps_;
}

DataSample BatchData::GetSample(std::size_t n) const {
  if (n >= GetDataSize())
